...
use iroh::net::endpoint::{Connection, Connecting};
...
// New: Start hosting and return the code immediately
...
// New: Wait for a client to connect (called after showing the code)
pub async fn wait_for_client(endpoint: Endpoint) -> Result<NetworkManager> {
    ...
    let connecting: Connecting = incoming.accept()?;
...
pub async fn connect_as_host() -> Result<(NetworkManager, String)> {
    let (endpoint, code) = start_hosting().await?;
    let manager = wait_for_client(endpoint).await?;
    Ok((manager, code))
}
